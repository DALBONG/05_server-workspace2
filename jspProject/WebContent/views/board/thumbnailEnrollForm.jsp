        ...
        <form action="<%= contextPath %>/insert.th" id="enroll-form" method="post" enctype="multipart/form-data">
            ...
            <table align="center">
                ...
                <tr>
                    ...
                    <td colspan="3" align="center">
                        <img id="titleImg" width="250" height="170" align="center" onclick="chooseFile(1)">
                    </td>
                </tr>
            ...
            <script>
                ...
                function loadImg(inputFile, num){
                    ...
                    // num : 몇번째 input요소인지 확인 후 해당 그 영역에 미리보기 하기 위해 전달받는 숫자
                    
                    // 선택된 파일이 있다면 inputFile.files[0]에 선택된 파일이 담겨있음
                                     // ㄴ files라는 배열은 내부적으로 만들어 준 것. 
                                     // inputFile.files.length 또한 1이 될 것.
                    if(inputFile.files.length == 1){ 
                        // 파일이 선택된 경우 -> 파일 읽고 미리보기
                        
                        // 파일 읽을 객체 생성 (FileReader) -> 객체의 메소드 사용하려고
                        const reader = new FileReader();
                        
                        // 파일 읽어들일 메소드 호출.(readAsDataURL)
                        reader.readAsDataURL(inputFile.files[0]);
                                     // 해당 파일을 읽는 순간 그 파일의 고유한 url 부여하는 메소드.

                        // 파일 읽기 완료 되었을 경우 실행할 함수 정의
                        reader.onload = function(e){
                                    //모든 이벤트 핸들러는 e라는 객체를 가짐
                                    // e.target.resule -> 읽은 파일의 고유한 url을 가져오는 메소드.
                            switch(num){
